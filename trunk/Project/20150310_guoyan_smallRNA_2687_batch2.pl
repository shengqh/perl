...
my $config_mouse = {
  ...
  bowtie1_genome_1mm_notidentical => {
    ...
    source_config_ref => [ $config_human, "cutadapt", "2687-GG-10.*.fastq.gz\$" ],
  ...
  bowtie1_genome_1mm_NTA => {
    ...
    option            => $def_mouse->{bowtie1_option_1mm},
    source_config_ref => [ $config_human, "identical_NTA", "2687-GG-10.*.fastq.gz\$" ],
    bowtie1_index     => $def_mouse->{bowtie1_index},
    samonly           => 0,
    sh_direct         => 1,
  ...
  bowtie1_genome_1mm_NTA_smallRNA_count => {
    ...
    fastq_files_config_ref => [ $config_human, "identical_NTA", "2687-GG-10.*.fastq.gz\$"  ],
    seqcount_config_ref    => [ $config_human, "identical_NTA", "2687-GG-10.*.dupcount\$" ],
