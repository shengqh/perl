...
my $def_human = {
  ...
  target_dir => "/scratch/cqs/shengq1/vickers/20150224_smallRNA_3018-KCV-10/human",
  max_thread => 8,
...
my $def_cow = {
  ...
  task_name  => "3018-KCV-10",
  email      => "quanhu.sheng\@vanderbilt.edu",
  target_dir => "/scratch/cqs/shengq1/vickers/20150227_smallRNA_3018-KCV-10/cow",
  max_thread => 8,
  cluster    => "slurm",
...
my $config_cow = {
  ...
  bowtie1_genome_pm_count => {
    ...
    option              => "-s --min_overlap 0.6 -l 18 --orientation_free --gtf_key exon",
...
my $def_bacteria = {
  ...
  coordinate    => "/scratch/cqs/shengq1/references/bacteria/NC_012803.bed",
  bowtie1_index => "/scratch/cqs/shengq1/references/bacteria/bowtie_index_1.1.1/NC_012803",
};

my $config_bacteria = {
  general => { task_name => $def_bacteria->{task_name}, },

  bowtie1_genome_pm => {
    class             => "Alignment::Bowtie1",
    perform           => 1,
    target_dir        => $def_bacteria->{target_dir} . "/bowtie1_genome_pm",
    option            => $def_bacteria->{bowtie1_option_pm},
    source_config_ref => [ $config_human, "bowtie1_genome_1mm_NTA_smallRNA_count", "unmapped.fastq.gz\$" ],
    bowtie1_index     => $def_bacteria->{bowtie1_index},
    samonly           => 0,
    sh_direct         => 1,
  ...
  bowtie1_genome_pm_count => {
    class               => "CQS::CQSMappedCount",
    perform             => 1,
    target_dir          => $def_bacteria->{target_dir} . "/bowtie1_genome_pm_count",
    option              => "-s --min_overlap 0.6 -l 18 --orientation_free --gtf_key exon",
    source_ref          => "bowtie1_genome_pm",
    seqcount_config_ref => [ $config_human, "identical", ".dupcount\$" ],
    cqs_tools           => $def_bacteria->{cqstools},
    gff_file            => $def_bacteria->{coordinate},
    samtools            => $def_bacteria->{samtools},
    sh_direct           => 1,
    cluster             => $def_bacteria->{cluster},
    pbs                 => {
      "email"    => $def_bacteria->{email},
  ...
  bowtie1_genome_pm_table => {
    class      => "CQS::CQSMappedTable",
    perform    => 1,
    target_dir => $def_bacteria->{target_dir} . "/bowtie1_genome_pm_table",
    option     => "",
    source_ref => [ "bowtie1_genome_pm_count", ".mapped.xml" ],
    cqs_tools  => $def_bacteria->{cqstools},
    prefix     => "bacteria_pm_",
  ...
  sequencetask => {
    class      => "CQS::SequenceTask",
    perform    => 0,
    target_dir => $def_bacteria->{target_dir} . "/sequencetask",
    option     => "",
    source     => {
      one => [ "bowtie1_genome_pm", "bowtie1_genome_pm_count" ],
      all => [
